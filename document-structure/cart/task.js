...
function addToCart(evt) {
    let isNew = true; //новый товар (да/нет)
    let cartProduct = null;//задаем товар без значения
    ...
    //проверяем корзину
    [...productsList.children].forEach((cartItem) => {
        if (cartItem.dataset.id === productId) { //если в корзине уже есть товар с таким же id как у нового товара, который хотим добавить
            isNew = false;//это не новый товар
            cartProduct = cartItem;//значит новый товар равен тому товару что уже есть в корзине
        }
    });

    //если товар новый(его нет в корзине)
    if (isNew) {
